  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header Modal */}
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-sky-950 text-white px-6 py-4 flex items-center justify-between border-b border-white/10 dark:border-slate-800">
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-sm text-slate-800 dark:text-slate-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a
              ...
              className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-sky-400 dark:hover:border-sky-500 bg-slate-50 dark:bg-slate-800/80 hover:bg-sky-50/50 dark:hover:bg-slate-800 transition group flex items-start gap-3"
            >
              <Github className="w-5 h-5 text-slate-700 dark:text-slate-300 group-hover:text-sky-700 dark:group-hover:text-sky-400 flex-shrink-0 mt-0.5" />
              <div className="min-w-0">
                <p className="text-xs font-bold text-slate-800 dark:text-slate-200 group-hover:text-sky-900 dark:group-hover:text-sky-300 flex items-center gap-1">
                  Repository GitHub <ExternalLink className="w-3 h-3" />
                </p>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">pmonaldi68/ProgrammaGareCynthia1920_2026</p>
              </div>
            </a>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-emerald-400 dark:hover:border-emerald-500 bg-slate-50 dark:bg-slate-800/80 hover:bg-emerald-50/50 dark:hover:bg-slate-800 transition group flex items-start gap-3"
            >
              <ExternalLink className="w-5 h-5 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">pmonaldi68.github.io/ProgrammaGare...</p>
              </div>
            </a>
          </div>
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-white dark:bg-slate-850 space-y-2">
            ...
            <ol className="list-decimal pl-5 text-xs text-slate-600 dark:text-slate-300 space-y-1">
              ...
              <li>Il workflow <code className="text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-slate-800 px-1 py-0.5 rounded">.github/workflows/deploy.yml</code> già incluso nel progetto pubblicherà automaticamente il sito ad ogni commit su <code>main</code>!</li>
            </ol>
          </div>
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-white dark:bg-slate-850 space-y-2">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Il file <code className="text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-slate-800 px-1 py-0.5 rounded">.github/workflows/sync-sheet.yml</code> (incluso nel repository) si avvia automaticamente secondo una pianificazione cron e scarica il file CSV del foglio Google:
            </p>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              >
                {copiedSection === 'git' ? (
                  <>
                    <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Copiato!
                  </>
                ) : (
                  <>
            ...
            <pre className="bg-slate-900 dark:bg-slate-950 text-slate-100 p-3 rounded-xl text-xs font-mono overflow-x-auto border border-slate-800">
              {gitCommands}
            </pre>
          </div>

          {/* Versione pura HTML/JS */}
          <div className="bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800 rounded-xl p-3.5 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2.5">
            <FileCode className="w-4 h-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
            <div>
              <p className="font-bold text-amber-950 dark:text-amber-100">Versione Single-Page HTML/JS puro inclusa:</p>
        ...
        {/* Footer */}
        <div className="bg-slate-50 dark:bg-slate-850 px-6 py-3.5 border-t border-slate-200 dark:border-slate-800 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600 text-white transition shadow-sm"
          >
            Chiudi Guida
          </button>
        </div>
